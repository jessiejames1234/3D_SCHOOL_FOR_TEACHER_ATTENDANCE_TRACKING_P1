...
export default function AttendanceIndex() {
  ...
  React.useEffect(() => {
    const measurePing = async () => {
      ...
      let label, color;
      if (clamped < 100) { label = 'Good'; color = '#10b981'; }
      else if (clamped < 200) { label = 'Fair'; color = '#f59e0b'; }
  ...
  return (
    <div className="attendance-container">
      {/* EMBEDDED MODERN CSS STYLESHEET (No Vite/external css file needed) */}
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .gps-btn:hover { background-color: #475569; }
        .qr-btn {
          background-color: var(--accent-primary);
          color: #ffffff;
          box-shadow: 0 4px 12px rgba(59, 130, 246, 0.3);
        }
        .qr-btn:hover { background-color: var(--accent-hover); }
        ...
        .status-banner, .error-banner, .success-banner, .info-banner, .warning-banner, .error-alert, .error-banner-box {
          padding: 14px 18px;
          border-radius: 12px;
          font-size: 0.95rem;
          line-height: 1.5;
        ...
        .status-banner { background: #1e1b4b; border: 1px solid #4338ca; color: #c7d2fe; }
        .success-banner { background: #064e3b; border: 1px solid #059669; color: #a7f3d0; margin-top: 6px; }
        .error-banner, .error-alert, .error-banner-box { background: #450a0a; border: 1px solid #dc2626; color: #fecaca; }
        .info-banner { background: #0c4a6e; border: 1px solid #0284c7; color: #bae6fd; }
        .warning-banner { background: #451a03; border: 1px solid #d97706; color: #fde68a; }
        .test-user-btn {
          background: #ef4444; color: #fff; border: none; padding: 6px 12px; border-radius: 6px; cursor: pointer; font-weight: 600;
        }
        .attendance-grid {
          display: grid;
        ...
        .clear-qr-btn:hover { background: #ef4444; color: #fff; }
        .gps-mini-debug {
          background: #1e293b;
          border: 1px solid var(--border-slate);
          border-radius: 12px;
      ...
      <div className="attendance-card">
        ...
        <div className="attendance-header">
          ...
          <div className="header-actions">
            {!coords && !errorMessage && <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8', marginRight: 10 }}>Waiting for location...</p>}
            {!coords && !errorMessage && (
          ...
          <div className="attendance-left">
            ...
            <div className="records-list">
              {filteredRecords.length > 0 ? filteredRecords.map(item => {
                ...
                return (
                  <div key={item.attendance_id || `${item.schedule_id}-${item.date}`} className={`record-card ${isActive ? 'record-card-active' : ''}`}>
                    <div className="record-header">{item.date} ({item.day_of_week || ''})</div>
                    {isActive && <div className="active-schedule-note">Current active schedule</div>}
                    {parallelRooms.length > 0 && (
                      <div className="parallel-room-note">
          ...
          <div className="attendance-right">
            <div className="status-panel">
              ...
              <div className="qr-status-box">
                {!scannedQrToken ? (
                  ...
                  <div className="qr-text-active">
                      ...
                      {(!scannedFloorInRange && altDetectedFloor) && (
                        <div style={{ marginTop: 6, fontSize: 12, color: '#38bdf8' }}>
               ...
               {connectionQuality && (
                 <div className="gps-mini-debug" style={{ marginTop: 14 }}>
                   <div className="panel-label">WiFi SPEED</div>
                   <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                     {connectionQuality.label === 'No Signal' ? (
                       <i className="bi bi-wifi-off" style={{ fontSize: 18, color: connectionQuality.color }}></i>
                     ) : (
                       <i className={`bi ${connectionQuality.rtt < 100 ? 'bi-wifi' : connectionQuality.rtt < 200 ? 'bi-wifi-2' : 'bi-wifi-1'}`} style={{ fontSize: 18, color: connectionQuality.color }}></i>
                     )}
                     <span style={{ fontWeight: 600, color: connectionQuality.color }}>{connectionQuality.label}</span>
                     <span style={{ color: '#94a3b8' }}>{connectionQuality.rtt !== null ? `${connectionQuality.rtt}ms` : ''}{connectionQuality.effectiveType && connectionQuality.effectiveType !== 'offline' ? `, ${connectionQuality.effectiveType}` : ''}</span>
                   </div>
                 </div>
               )}
               ...
               {coords && (
                 <div className="gps-mini-debug" style={{ marginTop: 14 }}>
                   <div className="panel-label">GPS STATUS</div>
        ...
        <Modal show={isCameraVisible} title="Scan QR Code" onClose={() => setIsCameraVisible(false)}>
          <div style={{ position: 'relative', minWidth: 300, minHeight: 300 }}>
            ...
            <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', width: 250, height: 250, border: '2px solid #10b981', pointerEvents: 'none', borderRadius: 6 }}></div>
          </div>
        </Modal>
