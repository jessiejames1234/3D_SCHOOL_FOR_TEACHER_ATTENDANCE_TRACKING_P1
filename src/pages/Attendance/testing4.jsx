...
export default function AttendanceIndex() {
  ...
  const getCheckStatus = (record, checkType) => {
    ...
    if (checkType === 'in') {
      if (record.time_in) return { label: getFlagLabel(record.flag_in_id), flagId: record.flag_in_id };
      return { label: 'Upcoming', flagId: 1 };
    }
    if (checkType === 'mid') {
      if (record.time_check) return { label: getFlagLabel(record.flag_check_id), flagId: record.flag_check_id };
      return { label: 'Upcoming', flagId: 1 };
    }
    if (checkType === 'out') {
      if (record.time_out) return { label: getFlagLabel(record.flag_out_id), flagId: record.flag_out_id };
      return { label: 'Upcoming', flagId: 1 };
    }
